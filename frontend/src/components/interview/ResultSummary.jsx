    ...
    <section className="workspace-grid">
      <article className="panel">
        <div className="panel__header">
          <div>
            <h3 className="panel__title">면접 요약</h3>
            <p className="panel__subtitle">세션 진행 현황과 완료 상태를 확인할 수 있습니다.</p>
          </div>
      ...
      <article className="panel">
        <div className="panel__header">
          <div>
            ...
            <p className="panel__subtitle">저장된 답변과 세션 피드백을 기준으로 요약했습니다.</p>
          </div>
      ...
      <article className="panel panel--wide">
        <div className="panel__header">
          <div>
            ...
            <p className="panel__subtitle">다음 면접 전에 우선 확인하면 좋은 항목입니다.</p>
          </div>
        </div>
        ...
        <p className="panel__subtitle"><strong>추천 답변</strong> {result.feedback?.recommendedAnswer ?? '-'}</p>
        {highlights.length > 0 ? (
          <div className="resource-list">
            {highlights.map((item, index) => (
              <div key={`${item}-${index}`} className="resource-list__item resource-list__item--static">
                <strong>요약 {index + 1}</strong>
                <span>{item}</span>
              </div>
            ))}
          </div>
      ...
      <article className="panel panel--wide">
        <div className="panel__header">
          <div>
            <h3 className="panel__title">학습 연계 제안</h3>
        ...
        <div className="resource-list">
          ...
          )) : <div className="resource-list__item resource-list__item--static"><strong>추천 학습 없음</strong><span>추가 학습 추천 데이터가 아직 없습니다.</span></div>}
        </div>
      </article>
      ...
      <article className="panel panel--wide">
        <div className="panel__header">
          <div>
            ...
            <p className="panel__subtitle">질문, 답변 여부, 저장된 답변 내용을 확인할 수 있습니다.</p>
          </div>
        </div>
        <div className="resource-list">
          {items.map((item) => (
            <div key={item.id} className="resource-list__item resource-list__item--static">
              ...
              <span>{item.answered ? `답변 길이 ${item.answerLength ?? 0}자` : '답변 미작성'}</span>
              <span>{item.answerText || '아직 저장된 답변이 없습니다.'}</span>
            </div>
          ))}
        </div>
      </article>
